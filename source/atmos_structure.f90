  module atmos_structure
    ...
    use composition,     only : gas_molarconc,iGas_H2O, gas_name, cp_H2, cp_N2, cp_O2, cp_He, cp_O3, cp_CH4
    use thermodynamics,  only : get_Tsat_H2O, get_Tsat_H2O_GB, get_Tsat_CO2, get_psat_H2O

    use atmos_profile,   only : make_Tp_profile

    use fund_consts,     only : Rstar,pi        ! for atmos_profile. a little messy.
    ...
    public :: atmos_structure_init, set_Tadia_fvar, initialized
  ...
  contains
    ...
    subroutine atmos_structure_init(verbose_output, kappa_gray, Ts, Tlay, Tlev, play, plev, &
         dp, ps, f_i, mu_i, mu_avg, grav, cp_heat, hybrid,tanh_bool,weight_factor,flag)

      use fund_consts,     only : Rstar, stefan
      use composition,     only : composition_init
      use composition,     only : iGas_CO2, iGas_N2, iGas_H2, iGas_O3, mu_CO2, mu_N2, mu_H2O, mu_H2, mu_O3, iGas_O2, iGas_He, iGas_O3
      use composition,     only : iGas_CH4, iGas_SO2, iGas_H2S, iGas_NH3, mu_CH4, mu_SO2, mu_H2S, mu_NH3, mu_N2, mu_O2, mu_He, cpCO2
      ...
      real(8) plev_temp(nLev), lin_plev_temp(nLev)! pressure levels [Pa]
      ...
      real(8) logp1, logp2, pmin, pmax            ! assorted parameters for defining T-p profile []
      ...
      ! ### new ###
      ! variable for hybrid scheme
      real(8) cp_temp
      real(8) lin_plev(nLev) 
      real(8) wlog(nLev), wlin(nLev)
      real(8) dp_lin(nLay)
      real(8) w_sigma, w_x0
      real(8) play_prior(nLay), Tlay_prior(nLay)
      ...
      logical, intent(out) :: tanh_bool
      real(8), intent(out) :: weight_factor
      integer, intent(out) :: flag(1:nlayF)
   

      namelist/atmos_structure_nml/ps,Ts,ptop,Tstra,grav,RH,condense_CO2,use_atmos_profile,pv_atmos_profile,hybrid,tanh_bool,weight_factor
      ...
      if(iGas_O2/=-1)  call set_f_i_mu_i(iGas_O2, mu_O2, mu_i(iGas_O2), f_i(:,iGas_O2) )
      ...
      if(iGas_He/=-1) call set_f_i_mu_i(iGas_He,mu_He,mu_i(iGas_He),f_i(:,iGas_He))
      ...
      ! ### new ###
      ! add methane
      elseif(iGas_CH4>-1)then
         if(gas_molarconc(iGas_CH4)>0.8d0)then
            cp_dry = 2181
         endif
      ...
      ! ### new ###
      ! add helium
      elseif(iGas_He>-1)then
         if(gas_molarconc(iGas_He)>0.8d0)then
            cp_dry = 5192.0d0
         endif
      else
      ...
      !!! determine total heat capacity (excluding condensable)
      cp_heat = 0.0d0
      cp_temp = 0.0d0

      do iGas=1,nGas
         if (gas_molarconc(iGas).gt.0.0d0)then
            ...
            cp_heat = cp_heat + gas_molarconc(iGas)*cp_temp
            cp_temp = 0.0d0
         end if
      end do
      ...
      ! logarithmic scaling
      do iLev = 1,nLev
         logplev    = (-1.0d0)*dble(iLev-1)*(logp2 - logp1)/dble(nLev-1)
         plev(iLev) = ps*1.0d1**logplev
      end do
      ...
      if(hybrid)then
         ...
         ! linear scaling
         do iLev=1,nLev
            lin_plev(iLev) = pmax - dble(iLev-1)*(pmax - pmin)/dble(nLev-1) !pmin + dbl*(pmax - pmin)/dbl
         end do

         if(tanh_bool)then
            ! tanh scheme
            w_x0 = int((nLev-1)*weight_factor)
         else
            ! sin scheme
            w_x0 = pi*weight_factor/(nLev-1)
         endif

         do iLev = 1,nLev
            ! tanh scheme
            if(tanh_bool)then
               wlin(iLev) = 0.5d0*(1+tanh((iLev - w_x0)/(w_sigma*(nLev-1))))
               wlog(iLev) = 1.0d0 - wlin(iLev)
            ! sin scheme
            else
               wlog(iLev) = (1+sin(iLev*w_x0))/2
               wlin(iLev) = 1.0d0 - wlog(iLev)
            endif
            ! hybrid scaling
            plev(iLev) = wlog(iLev)*plev(iLev) + wlin(iLev)*lin_plev(iLev)
         end do
      ...
      if(use_atmos_profile)then
         ...
         call make_Tp_profile(condense_CO2,variable_cp,mu_avg_dry/1.0d3,mu_H2O/1.0d3,R_dry,cp_tot,&
            Tstra,Ts,ptop,ps,ps_v,play,plev,Tlay,Tlev,f_i(:,vgas),hybrid,tanh_bool,weight_factor,flag)
    ...
    subroutine set_Tadia_fvar(ps,Ts,Tlay,Tlev,play,plev,dp,Tlay_adia,Tlev_adia,f_i,hybrid,tanh_bool,weight_factor,flag)
      USE, INTRINSIC :: IEEE_ARITHMETIC            ! debugging
      use composition, only : iGas_H2O, iGas_CO2, mu_CO2, mu_H2O

      implicit none

      real(8), intent(inout) :: ps                  ! surface pressure [Pa]
      real(8), intent(inout) :: Ts                  ! surface temperature [K]
      real(8), intent(inout) :: Tlay(nLay)          ! temperature layers [K]
      real(8), intent(inout) :: Tlev(nLev)          ! temperature layers [K]
      ...
      logical, intent(in) :: tanh_bool
      real(8), intent(in) :: weight_factor
      integer, intent(out) :: flag(1:nlayF)
      ...
      atm_prof_if : if(use_atmos_profile)then
         ...
         call make_Tp_profile(condense_CO2,variable_cp,mu_avg_dry/1.0d3,mu_H2O/1.0d3,R_dry,cp_tot,&
            Tstra,Ts,ptop,ps,ps_v,play,plev,Tlay_adia,Tlev_adia,f_i(:,vgas),hybrid,tanh_bool,weight_factor,flag)
