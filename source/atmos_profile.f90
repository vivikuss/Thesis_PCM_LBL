...
contains  
  ...
  subroutine make_Tp_profile(condense_CO2,variable_cp,m_n,m_v,Rmn,cp_n_in,Tstra,Ts,ptop,ps,Psurf_v,play,plev,Tlay,Tlev,flay,hybrid,tanh_bool,weight_factor,flag)
    ...
    use fund_consts, only : Rstar, pi
    use composition, only : gas_name, vgas, iGas_H2O, iGas_CO2
    ...
    logical, intent(in) :: hybrid          ! ### new ### hybrid scheme used?
    logical, intent(in) :: tanh_bool       ! ### new ### tanh or sin hybrid scheme? true -> tanh, false -> sin
    real(8), intent(in) :: weight_factor   ! ### new ### parameter for hybrid scheme
    integer, intent(out) :: flag(nLay)     ! 0 = dry, 1 = moist, 2 = isothermal for output
    ...
    real(8) dp_lin(nLay), dp_arr(nLay)
    real(8) w_sigma, w_x0
    real(8) lin_plev(nLev), lin_plev_temp(nLev), plev_temp(nLev)
    real(8) log_dp, logp1, logp2, logplev
    ...
    if(hybrid)then
         ...
         do iLev=1,nLev
            lin_plev(iLev) = pmax - dble(iLev-1)*(pmax - pmin)/dble(nLev-1) 
         end do
    ...
    do iLayF=2,nlayF
       ...
       if(iLay<=nLay)then
          ...
          if(Pn(iLayF)+Pv(iLayF) < play(iLay))then
             ...
             flag(iLay) = profil_flag(iLayF)            ! ### new ### return flag to main                                     !!!!!!!!!!!!!!!!!!!!!!!!!
             iLay = iLay + 1
