...
contains
  ...
  subroutine setup_cia(pairtype)
    ...
    if(pairtype=='H2__H2')then
       ipair     = 1
       nT(ipair) = 10
       nS(ipair) = 9981          ! ### new ### updated cia data
    ...
    elseif(pairtype=='CO2CH4')then
       ipair     = 7
       nT(ipair) = 4
       nS(ipair) = 401                 ! ### new ### updated cia data
    ...
    ! ### new ### added cia for CO2/He
    elseif(pairtype=='CO2He_')then
         ipair     = 10
         nT(ipair) = 1
         nS(ipair) = 1000
    else
    ...
    if(ipair==1)then
       dt_file=trim(datadir)//'cia/H2-H2_2011.cia'   ! ### new ### updated cia data
    elseif(ipair==2)then
    ...
    elseif(ipair==7)then
       dt_file=trim(datadir)//'cia/CO2-CH4_2018.cia'        ! ### new ### updated cia data
       !write(*,*) 'set path '
    ...
    elseif(ipair==8)then
      ...
      ! ### new ### added cia for CO2/He
      elseif(ipair==10)then
         dt_file=trim(datadir)//'cia/CO2-He_2018.cia'
    endif

    if(ipair<6 .or. (ipair == 6 .and. use_turbet_data.eqv..true.) .or. ipair.eq.7 .or. ipair == 8 .or. ipair.eq.10)then
       ...
       ! ### new ### 
       ! added cia for CO2/He
       if(ipair.eq.10)then
         call calculate_cia(pairtype,200.0d0,300.0d0,1.0d5,1.0d5,1.0d05,blah,.true.)
       else
  ...
  subroutine interpolateH2Ocont_PPC(wn,temp,presS,abcoef)
    ...
    use composition, only: mu_H2O, vgas
